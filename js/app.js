(() => {
    ...
    function initSliders() {
        if (document.querySelector(".previous__slider")) {
            new core(".previous__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3.5,
                ...
                speed: 1e3,
                navigation: {
                    prevEl: ".previous__arrow_prev",
                    nextEl: ".previous__arrow_next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
    ...
    da.init();
    const filterButtons = document.querySelectorAll(".filter-previous__button");
    const slides = document.querySelectorAll(".previous__slide");
    let activeButton;
    function removeActiveClass() {
        if (activeButton) activeButton.classList.remove("active");
    }
    for (const button of filterButtons) button.addEventListener("click", (e => {
        const filter = button.dataset.filter;
        for (const slide of slides) if ("all" === filter || slide.classList.contains(filter)) slide.style.display = "block"; else slide.style.display = "none";
